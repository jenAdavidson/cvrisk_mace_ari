...
CODELISTS:				PermCMI_Aurum_Mar20.dta
...
*************************************************************
***APPLY EXCLUSIONS - PERMANENT CELLULAR IMMUNE DEFICIENCY***
*************************************************************
...
forvalues y=1/10 {
	...
	merge m:1 medcodeid using "$codelistdir\PermCMI_Aurum_Mar20.dta", keep(match) nogen
	if _N == 0 continue
	
	*CREATE EARLIEST DATE FOR ANY DIAG & DROP IF BEYOND STUDY PERIOD END
	sort patid obsdate
	by patid: egen permcmidate=min(obsdate) 
	...
	*ONLY KEEP ONE OBSERVATION PER PATIENT, THE ONE WITH THE EARLIEST EVENT DATE
	...
	tempfile observation_PermCMI_`x'_`y'
	save `observation_PermCMI_`x'_`y'', replace
	}
	}

forvalues x=1/9 {
use `observation_PermCMI_`x'_1', clear
forvalues y=2/10 {
	capture noisily append using `observation_PermCMI_`x'_`y''
}	
	if _rc==111{
	continue
	}
	...
	tempfile observation_permcmi_`x'
	save `observation_permcmi_`x'', replace	
	}
	
	use `observation_permcmi_1', clear	
	forvalues x=2/9 {
	append using `observation_permcmi_`x''
	}	
	
save "$intermediatedatadir\observation_PermCMI.dta", replace